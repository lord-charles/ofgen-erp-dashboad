...
} from "@/services/subcontractors.service";
...
const subcontractorSchema = z
  ...
  .refine(
    ...
    {
      message:
        "Company name is required for companies, first and last name are required for individuals",
      path: ["companyName"],
    }
  );
  ...
  const handleSubmit = async (data: SubcontractorFormData) => {
    ...
    try {
      ...
      const apiData: CreateSubcontractorDto = {
        ...
        skills: Array.isArray(formDataWithoutRating.skills)
          ? formDataWithoutRating.skills
          : [],
      };
      ...
      toast({
        title: "Error",
        description:
          error?.message || "Failed to add subcontractor. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              >
                                ...
                                <SelectContent>
                                  {Object.entries(SubcontractorSpecialty).map(
                                    ([key, value]) => (
                        ...
                        render={({ field }) => (
                          <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                            <div className="space-y-0.5">
                              ...
                              <div className="text-sm text-muted-foreground">
                                {field.value
                                  ? "Active and available for projects"
                                  : "Inactive"}
                              </div>
                            </div>
                            <FormControl>
