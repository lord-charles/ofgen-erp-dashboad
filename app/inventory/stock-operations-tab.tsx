...
import { Plus, Settings, TrendingUp, TrendingDown, Package, AlertTriangle } from "lucide-react"
...
const API_BASE = "https://ofgen.cognitron.co.ke/ofgen/api"
const AUTH_TOKEN =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI2ODI3NDRjODkwOThkZDU3M2YzMDdmMzkiLCJlbWFpbCI6ImphbmUud2FuamlrdUBjb21wYW55LmNvbSIsInJvbGVzIjpbImVtcGxveWVlIl0sImlhdCI6MTc0ODU5MDcxNiwiZXhwIjoxNzgwMTI2NzE2fQ.1a10QLZHe0_Gdnt6e7KWP9TM_qrE2JZh4t4yrtDBI_c"
...
export function StockOperationsTab() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [itemsData, warehousesData] = await Promise.all([itemsRes.json(), warehousesRes.json()])
  ...
  const handleStockAdjustment = async (e: React.FormEvent) => {
    ...
    if (!adjustmentData.itemId || !adjustmentData.locationId || !adjustmentData.reason) {
      toast({
  ...
  const handleStockReservation = async (e: React.FormEvent) => {
    ...
    if (!reservationData.inventoryItem || !reservationData.location || reservationData.quantity <= 0) {
      toast({
        title: "Validation Error",
    ...
    try {
      const response = await fetch(`${API_BASE}/inventory/stock/reserved`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${AUTH_TOKEN}`,
          "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        throw new Error(data.message || "Failed to update stock reservation")
      }
    } catch (error) {
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to update stock reservation",
        variant: "destructive",
      })
    }
  }
  ...
  const openAdjustmentDialog = () => {
    if (items.length === 0) {
      toast({
        title: "No Items",
        description: "Please create inventory items before making stock adjustments",
        variant: "destructive",
      })
      return
    }
  ...
  const openReservationDialog = () => {
    if (items.length === 0) {
      toast({
        title: "No Items",
        description: "Please create inventory items before managing reservations",
        variant: "destructive",
      })
      return
    }
  ...
  const getCurrentStock = () => {
    ...
    const stockLevel = selectedItem.stockLevels.find((sl) => sl.location._id === selectedLocation)
  ...
  if (loading) {
    return <div className="flex justify-center p-8">Loading stock operations...</div>
  }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Adjust inventory levels for damaged, lost, or found items</CardDescription>
          </CardHeader>
          <CardContent>
            <Dialog open={isAdjustmentDialogOpen} onOpenChange={setIsAdjustmentDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Adjust stock levels for inventory corrections, damage, or other reasons.
                  </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleStockAdjustment}>
                  <div className="grid gap-4 py-4">
                    <div className="grid gap-2">
                      ...
                      <Select value={adjustmentData.itemId} onValueChange={(value) => handleItemSelection(value, true)}>
                        <SelectTrigger>
                    ...
                    {selectedItem && (
                      <div className="grid gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value) => handleLocationSelection(value, true)}
                        >
                          <SelectTrigger>
                          ...
                          <SelectContent>
                            {selectedItem.stockLevels.map((level) => (
                              <SelectItem key={level.location._id} value={level.location._id}>
                                {level.location.name} - Current: {level.currentStock}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                    ...
                    {selectedLocation && (
                      <div className="p-3 bg-muted rounded-lg">
                        <div className="text-sm font-medium mb-2">Current Stock Information</div>
                        <div className="grid grid-cols-3 gap-2 text-sm">
                          <div>
                            <div className="text-muted-foreground">Total</div>
                            <div className="font-medium">{getCurrentStock()}</div>
                          </div>
                          <div>
                            <div className="text-muted-foreground">Available</div>
                            <div className="font-medium text-green-600">{getAvailableStock()}</div>
                          </div>
                          <div>
                            <div className="text-muted-foreground">Reserved</div>
                            <div className="font-medium text-orange-600">{getReservedStock()}</div>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setAdjustmentData((prev) => ({ ...prev, documentRef: e.target.value }))}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Manage reserved stock for orders and allocations</CardDescription>
          </CardHeader>
          <CardContent>
            <Dialog open={isReservationDialogOpen} onOpenChange={setIsReservationDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Increase or decrease reserved stock for specific items and locations.
                  </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleStockReservation}>
                  <div className="grid gap-4 py-4">
                    <div className="grid gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => handleItemSelection(value, false)}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Select item" />
                        </SelectTrigger>
                    ...
                    {selectedItem && (
                      <div className="grid gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value) => handleLocationSelection(value, false)}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="Select location" />
                          </SelectTrigger>
                          <SelectContent>
                            {selectedItem.stockLevels.map((level) => (
                              <SelectItem key={level.location._id} value={level.location._id}>
                                {level.location.name} - Available: {level.availableStock}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      ...
                      <div className="grid gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value: "increase" | "decrease") =>
                            setReservationData((prev) => ({ ...prev, action: value }))
                          }
                        >
                          <SelectTrigger>
                            <SelectValue />
                  ...
                  <DialogFooter>
                    ...
                    <Button type="submit">
                      {reservationData.action === "increase" ? "Increase" : "Decrease"} Reservation
                    </Button>
                  </DialogFooter>
                </form>
              </DialogContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Current stock levels across all items and locations</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">Create inventory items to view stock operations.</p>
            </div>
          ) : (
                ...
                <Card key={item._id} className="p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {item.stockLevels.map((level) => (
                      <div key={level.location._id} className="border rounded-lg p-3">
                        <div className="font-medium text-sm mb-2">{level.location.name}</div>
                        <div className="grid grid-cols-3 gap-2 text-xs">
                          <div>
                            <div className="text-muted-foreground">Total</div>
                            <div className="font-medium">{level.currentStock}</div>
                          </div>
                          <div>
                            <div className="text-muted-foreground">Available</div>
                            <div className="font-medium text-green-600">{level.availableStock}</div>
                          </div>
                          <div>
                            <div className="text-muted-foreground">Reserved</div>
                            <div className="font-medium text-orange-600">{level.reservedStock}</div>
                          </div>
                        </div>
                      </div>
                    ))}
