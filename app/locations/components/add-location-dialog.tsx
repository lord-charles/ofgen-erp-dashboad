...
export function AddLocationDialog({ open, setOpen }: AddLocationDialogProps = {}) {
  const [internalOpen, setInternalOpen] = useState(false);
  const isCreateDialogOpen = open !== undefined ? open : internalOpen;
  const handleSetOpen = setOpen || setInternalOpen; // fallback to internal
  ...
  const handleCreateLocation = async (formData: FormData) => {
    ...
    try {
      const locationData = {
        ...
        siteType: formData.get("siteType") as
          | "outdoor"
          | "indoor"
          | "rooftop"
          | "ground",
  ...
  return (
    <Dialog open={isCreateDialogOpen} onOpenChange={handleSetOpen}>
      {open === undefined && (
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader className="space-y-3">
          ...
          <DialogDescription className="text-base">
            Create a new solar installation site location with comprehensive
            details
          </DialogDescription>
        </DialogHeader>
