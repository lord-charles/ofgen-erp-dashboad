...
const AddContractorDialog = ({ onAdd, trigger }: AddContractorDialogProps) => {
  ...
  const onSubmit = async (data: ContractorFormData) => {
    ...
    try {
      ...
      toast({
        ...
        description: error?.message || "An error occurred while adding the contractor.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <Tabs defaultValue="personal" className="w-full">
              ...
              {(() => {
                const personalFields: Array<keyof ContractorFormData> = ["firstName", "lastName", "nationalId", "dateOfBirth"];
                const contactFields: Array<keyof ContractorFormData> = ["email", "phoneNumber"];
                const employmentFields: Array<keyof ContractorFormData> = ["position", "employmentType", "employmentEndDate", "status", "roles"];
                const errors = form.formState.errors;
                const hasPersonalError = personalFields.some(f => !!errors[f]);
                const hasContactError = contactFields.some(f => !!errors[f]);
                const hasEmploymentError = employmentFields.some(f => !!errors[f]);
                const RedDot = () => <span className="ml-1 inline-block w-2 h-2 rounded-full bg-red-500 align-middle" />;
                return <>
